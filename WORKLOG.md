...
- [ ] 사장님: Vercel 환경변수 ESTIMATE_HOOK_URL·ESTIMATE_HOOK_SECRET 추가 → Redeploy → /pc-estimate/ 시험 요청으로 자동 초안 견적 텔레그램 도착 확인 (값은 bench_mark_danawa/auto/secrets.json 의 hook_secret)
...
- 2026-09-26 (커밋 아래) 자동 초안 견적: PC 견적 요청을 파이 견적 서버(/api/estimate-request, 비밀키)로 전달(연락처 제외). 파이 쪽 bench_mark_danawa/auto/ (worker.py cron 1분, quote_tool.py, RULES.md) 가 Claude 로 초안 → @bucom_homepage_bot 으로 링크.
